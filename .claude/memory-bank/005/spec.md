...
> На момент написания спеки таблица пустая — backfill не требуется.
> Уникальный индекс по `pandascore_id` добавляется без риска конфликтов.
...
## Обязательный охват реализации

Задача затрагивает 4 таблицы и 2 импортера — это осознанное решение (все поля нужны задачам 007–009 одновременно, разбиение создаст зависимость между PR-ами).

**Агент ДОЛЖЕН реализовать все 4 таблицы и оба импортера — частичная реализация недопустима.**

### Что НЕ входит в scope

- Удаление колонки `tournament` (после завершения задач 007–009, отдельная задача)
- Удаление колонки `played_at` (отдельный рефакторинг)
- Обновление `role` и `team_id` при трансфере игрока (выходит за рамки 005)
- Реализация auth
...
- Проверить, что игрок создан с конкретными значениями из fixture, например: `pandascore_id: 11234`, `role: "rifler"`.
...
- Проверить, что `MapResult` содержит конкретные значения из fixture, например: `pandascore_id: 301`, `score: "16-9"`, `winner_team_id: <id команды team_a из fixture>`.
...
| `game["id"]` = nil | Карта пропускается — `pandascore_id` обязателен для дедупликации |
